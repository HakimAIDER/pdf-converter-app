...
export const WordToPdfView: React.FC = () => {
  ...
  const handleConvert = () => {
    if (!file) {
      alert("Veuillez d'abord télécharger un fichier Word.");
      return;
    }
    setIsConverting(true);
    console.log("Converting Word to PDF...", file);
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 sm:p-8 shadow-2xl">
      <h2 className="text-2xl font-semibold mb-4 text-center text-slate-100">Word en PDF</h2>
      <WordUploader file={file} onFileChange={setFile} />
